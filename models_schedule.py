...
class Teacher:
    ...
    @staticmethod
    def find_or_create(raw_name):
        """
        Ищет преподавателя по ФИО с fuzzy-матчингом.
        Если не находит похожего — создаёт нового.

        Возвращает (teacher_id, raw_name, was_created, matched_name).
        """
        if not raw_name:
            return None, raw_name, False, None

        raw_name = raw_name.strip()

        # Точное совпадение
        existing = Teacher.get_by_full_name(raw_name)
        if existing:
            return existing['id'], raw_name, False, existing['full_name']

        # Fuzzy-матчинг через rapidfuzz
        try:
            from rapidfuzz import process, fuzz

            all_teachers = Teacher.get_all(active_only=False)
            if all_teachers:
                # Собираем и full_name, и short_name как варианты
                candidates = {}
                for t in all_teachers:
                    if t['full_name']:
                        candidates[t['full_name']] = t['id']
                    if t['short_name']:
                        candidates[t['short_name']] = t['id']
                ...
                if best and best[1] >= 85:
                    matched_name = best[0]
                    tid = candidates.get(matched_name)
                    if tid:
                        return tid, raw_name, False, matched_name
        except ImportError:
            # Fallback: difflib, если rapidfuzz не установлен
            import difflib
            all_teachers = Teacher.get_all(active_only=False)
            names = [t['full_name'] for t in all_teachers if t['full_name']]
            matches = difflib.get_close_matches(raw_name, names, n=1, cutoff=0.85)
            if matches:
                existing = Teacher.get_by_full_name(matches[0])
                if existing:
                    return existing['id'], raw_name, False, existing['full_name']

        # Не нашли — создаём нового
        ok, msg, new_id = Teacher.create(raw_name)
        if ok:
            return new_id, raw_name, True, raw_name

        # Если не смогли создать (например, гонка) — пробуем найти снова
        existing = Teacher.get_by_full_name(raw_name)
        if existing:
            return existing['id'], raw_name, False, existing['full_name']
